...
def trainer_addtopic(request):
    if 'usernametrnr2' in request.session:
        ...
        crt = create_team.objects.filter(trainer_id=uname.id)
        mem = topic()
        if request.method == 'POST':
...
def account_payment_salary(request,id):
    if 'usernameacnt2' in request.session:
        ...
        year = date.today().year
        month = date.today().month
        delays = project_taskassign.objects.filter(developer=vars.id,startdate__year__gte=year,
                                          startdate__month__gte=month,
                                          submitted_date__year__lte=year,
                                          submitted_date__month__lte=month)
        mm = delays.values_list('delay', flat='true')
        ...
        if request.method == "POST":
            ...
            abc.incometax = 0
            abc.leavesno = request.POST["leaves"]
        ...
        return render(request, 'account_payment_salary.html',{'vars':vars,'z' : z,'delays':delays, 'a':a})
    else:
        return redirect('/')
...
def BRadmin_accounts(request):
    if 'Adm_id' in request.session:
        ...
        des = designation.objects.get(designation='trainee')
        Adm1 = designation.objects.get(designation="Admin")
        deta3 = user_registration.objects.filter(designation=des.id).count()
        deta = user_registration.objects.filter(designation=des.id,payment_status=0).count()
        mem1 = user_registration.objects.filter(salary_status=1).count()
        var = user_registration.objects.filter(confirm_salary_status=1).order_by("-id").count()
        var1 = user_registration.objects.exclude(designation=des.id).exclude(designation=Adm1.id).order_by("-id").count()
        icount=internship.objects.all().count()
        year = date.today().year
        month = date.today().month
        giv = acntspayslip.objects.filter(fromdate__year=year,
                                          fromdate__month=month).count()
        pen = acntspayslip.objects.filter(~Q(fromdate__year=year,
                                          fromdate__month=month)).count()
        month = acnt_monthdays.objects.all().count() 
        return render(request, 'BRadmin_accounts.html', {'Adm': Adm, 'deta':deta, 'deta3':deta3, 'mem1':mem1, 'var':var, 'var1':var1,'icount':icount,'giv':giv,'pen':pen,'month':month})
    else:
        return redirect('/')
...
def BRadmin_accounts_internship(request):
    if 'Adm_id' in request.session:
        ...
        return render(request, 'BRadmin_accounts_internship.html', {'Adm': Adm, 'ipcount':ipcount, 'ivacount':ivacount,'ivdcount':ivdcount})
    else:
        return redirect('/')   
...
def BRadmin_accounts_internship(request):
    if 'Adm_id' in request.session:
        ...
        ipcount=internship.objects.filter(complete_status='0').count()
        ivacount=internship.objects.filter(complete_status=1).count()
        ivdcount=internship.objects.all().values('reg_date').distinct().count()
...
def BRadmin_monthdays(request):
    if 'Adm_id' in request.session:
        ...
        Adm = user_registration.objects.filter(id=Adm_id)
        mem = acnt_monthdays.objects.all()
        return render(request,'BRadmin_monthdays.html', {'Adm': Adm,'mem':mem})
    else:
        return redirect('/')
...
def projectmanager_emp_ajax(request):
    if 'prid' in request.session:
        ...
        dept_id = request.GET.get('dept_id')
        desigId = request.GET.get('desigId')
        Desig = user_registration.objects.filter(department=dept_id, designation=desigId)
...
def BRadmin_projects_details(request):
    if 'Adm_id' in request.session:
        ...
        return render(request,'BRadmin_projects_details.html', {'names':names,'mm':mm,'a':a ,})
    else:
        return redirect('/')
...
#     if request.method == 'POST':
#         tid = request.GET.get('tid')
#         abc = probation.objects.get(id=tid)
#         abc.name = request.POST.get('teams')
#         abc.trainer_id = request.POST.get('trainer')
#         tra= user_registration.objects.get(id=a)
#         abc.trainer = tra.fullname
